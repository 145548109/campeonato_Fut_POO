...
public class Torneo {
    ...
    public void programarPartidos() {
        if (equipos.size() % 2 == 1) {
            equipos.add(null); // Añadir un descanso si es necesario
        }
        int rondas = equipos.size() - 1;
        for (int ronda = 0; ronda < rondas; ronda++) {
            for (int i = 0; i < equipos.size() / 2; i++) {
                Equipo equipoA = equipos.get(i);
                Equipo equipoB = equipos.get(equipos.size() - i - 1);
                if (equipoA != null && equipoB != null) {
                    Partido partido = new Partido(equipoA, equipoB, null, null, null);
                    partidos.add(partido);
                }
            }
            equipos.add(1, equipos.remove(equipos.size() - 1)); // Rotar equipos
        }
    }
